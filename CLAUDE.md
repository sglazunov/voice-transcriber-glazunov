...
## Прод-сервер (с 2026-07-27)
- Проект развёрнут на **Ubuntu-сервере**, доступ по `http://<IP>:8000`.
  Установка — `bash deploy.sh` (ставит системные пакеты, venv, модель, systemd-юнит
  `voicetx` с текущим пользователем/путём). Инструкция: `docs/СЕРВЕР.md`.
- Обновление на сервере: `git pull && .venv/bin/pip install -r requirements.txt
  && sudo systemctl restart voicetx`. Логи: `journalctl -u voicetx -f`.
- **Код теперь заливается в Яндекс SourceCraft** (репозиторий переехал; URL
  уточнить у пользователя, `git remote set-url origin <url>`). GitHub остаётся
  как есть, пока не сказано иное.
- ⚠️ **Рекордер на Linux не работает**: `capture.py` — только Windows
  (`gdigrab`/`dshow`, `list_audio_devices` возвращает [] если не win32).
  Нужен бэкенд Xvfb + x11grab + PulseAudio null-sink. Это ближайшая задача.
- Ключи LLM ядра живут в памяти → на сервере после рестарта слетают; для
  постоянства прописывать через `systemctl edit voicetx` (Environment=...).
...
### ⚠️ Открытая задача: протокол «стал непонятным»
Обратная связь от коллег (2026-07-27): текущий протокол читается плохо.
Предыдущая правка (обезличивание + обобщение задач) решала одну проблему, но,
судя по отзывам, ухудшила читаемость. При работе над этим: не возвращать
спикеров (это осознанное решение пользователя), а разбираться с
структурой/формулировками — вероятные причины: слишком дробные темы,
канцелярит, дубли между разделами, потеря контекста на reduce-шаге.
Сначала посмотреть реальный плохой протокол, а не править промпт вслепую.
